...
function add_styles() {
	// General styles
	wp_enqueue_style( 'main', get_template_directory_uri() . '/style/main.css?v=1.0' );
	...
	if ( is_front_page() ) { // Homepage
		...
		wp_enqueue_style( 'homepage', get_template_directory_uri() . '/style/homepage.css?v=1.0' );
		wp_enqueue_style( 'animate', get_template_directory_uri() . '/node_modules/animate.css/animate.min.css' );
	}
	
	// Reviews
	if ( is_page('reviews') ) { // Reviews
		wp_enqueue_style( 'reviews', get_template_directory_uri() . '/style/reviews.css?v=1.0' );
	}
	...
	if ( is_page(
		...
		wp_enqueue_style( 'page-document', get_template_directory_uri() . '/style/page-document.css?v=1.0' );
	}
	...
	if ( is_page(
		...
		wp_enqueue_style( 'page-document', get_template_directory_uri() . '/style/cart.css?v=1.0' );
	}
	...
	if ( is_archive() ) {
		...
		wp_enqueue_style( 'reservation', get_template_directory_uri() . '/style/reservation-page/booking.css?v=1.0' );
	}
}
...
function add_scripts () {
	 ...
	 */
	// Utils library
	wp_enqueue_script('utils-besedka', $directory . '/js/lib/utils.js?v=1.0', array(), null, true);
	
	// State management
	wp_enqueue_script('store', $directory . '/js/lib/store.js?v=1.0', array('utils-besedka'), null, true);
	
	// Notification class
	wp_enqueue_script('notification', $directory . '/js/lib/Notification.js?v=1.0', array('jquery'), null, true);
	
	// Notify about cookies
	wp_enqueue_script('cookies-notification', $directory . '/js/cookies-notification.js?v=1.0', array('js-cookie', 'notification'), null, true);
...
//	wp_enqueue_script('uncompleted-message', $directory . '/js/show-uncomplete.js', array('notification', 'js-cookie'), null, true);
	...
	// Works with header
	wp_enqueue_script('header', $directory . '/js/header.js?v=1.0', array('jquery'), null, true);
	
	// ServerClient class
	wp_enqueue_script('server-client', $directory . '/js/lib/ServerClient.js?v=1.0', array('jquery'), null, true);
	
	// Check phone library
	wp_enqueue_script('libphonenumber', 'https://unpkg.com/libphonenumber-js@1.x/bundle/libphonenumber-min.js', array(), null, true);

	// Callback function
	wp_enqueue_script('callback', $directory . '/js/callback.js?v=1.0', array('jquery', 'libphonenumber', 'notification'), null, true);
	
	// Localize callback
	wp_localize_script('store', 'callbackText', array( 'text' => get_field('notification_callback_success', 18) )); // 18 - home page ID
	
	// Loader class
	wp_enqueue_script('loader', $directory . '/js/lib/Loader.js?v=1.0', array('jquery'), null, true);
	
	// Show loader on page loading
	wp_enqueue_script('page-loader', $directory . '/js/page-loader.js?v=1.0', array('loader'), null, true);
	...
	if ( is_archive() ) {
		...
		// Main handler
		wp_enqueue_script('popup-lib', $directory . '/js/lib/popup/Popup.js?v=1.0', array(
			'store',
			'slider-handler',
			'select-handler',
			'datetime-handler',
		...
		// Slider in PopUp
		wp_enqueue_script('slider-handler', $directory . '/js/lib/popup/Slider.js?v=1.0', array('jquery', 'slick'), null, true);
		
		// Selector in PopUp
		wp_enqueue_script('select-handler', $directory . '/js/lib/popup/SelectDuration.js?v=1.0', array('jquery', 'custom-select'), null, true);
		
		// Datetimepicker in PopUp handler
		wp_enqueue_script('datetime-handler', $directory . '/js/lib/popup/DatetimeHandler.js?v=1.0', array('jquery', 'datetimepicker'), null, true);
		
		// Server request in PopUp
		wp_enqueue_script('booking', $directory . '/js/lib/popup/Booking.js?v=1.0', array(), null, true);
		
		
		// MAP
		
		// Main handler
		wp_enqueue_script('map-handler', $directory . '/js/lib/map/MapHandler.js?v=1.0', array(
			'jquery',
			'map-popup',
			'map-filter',
			'loader'
		...
		wp_enqueue_script('map-popup', $directory . '/js/lib/map/MapPopup.js?v=1.0', array('jquery'), null, true);

		// Filter points on map
		wp_enqueue_script('map-filter', $directory . '/js/lib/map/MapFilter.js?v=1.0', array('jquery'), null, true);
		...
		wp_enqueue_script('reservation', $directory . '/js/reservation.js?v=1.0', array('map-handler', 'popup-lib'), null, true);
	...
	if ( is_front_page() ) { // Homepage
		...
		wp_enqueue_script('sticky-button', $directory . '/js/sticky-button.js?v=1.0', array('jquery'), null, true);
		
		// Starts homepage scripts
		wp_enqueue_script('homepage', $directory . '/js/homepage.js?v=1.0', array('jquery', 'slick', 'viewport-checker'), null, true);
	}
	...
	if ( is_page('checkout') ) {
		...
		wp_enqueue_script('checkout-validation', $directory . '/js/checkout-validation.js?v=1.0', array('jquery', 'mask-input'), null, true);
	}
}