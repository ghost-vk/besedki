...
function add_styles() {
	wp_enqueue_style( 'main', get_template_directory_uri() . '/dist/general.css?v=1.2.3' );
	
	if ( is_front_page() ) { // Homepage
		wp_enqueue_style( 'homepage', get_template_directory_uri() . '/dist/homepage.css?v=1.2.3' );
	} else if ( is_page('reviews') ) { // Reviews
		wp_enqueue_style( 'reviews', get_template_directory_uri() . '/dist/reviews.css?v=1.2.3' );
	} else if ( is_page(array('privacy', 'user-agreement', 'rent-agreement')) ) { // Documents
		wp_enqueue_style( 'page-document', get_template_directory_uri() . '/style/page-document.css?v=1.2.3' );
	} else if ( is_page(array('cart', 'checkout')) ) { // Woocommerce cart and checkout
		wp_enqueue_style( 'cart', get_template_directory_uri() . '/dist/checkout.css?v=1.2.3' );
	} else if ( is_shop() ) { // Booking page
		wp_enqueue_style( 'reservation-map', get_template_directory_uri() . '/dist/reservation.css?v=1.2.3' );
	}
...
function add_scripts () {
	...
	if ( is_front_page() ) { // Homepage
		wp_enqueue_script('homepage', $directory . '/dist/homepage.bundle.js?v=1.2.3', array('ref'), null, true);
	} else if ( is_shop() ) { // Booking page
		...
		wp_enqueue_script('reservation', $directory . '/dist/reservation.bundle.js?v=1.2.3', array(), null, true);
	} else if ( is_page('reviews') ) { // Reviews
		wp_enqueue_script('reviews', $directory . '/dist/reviews.bundle.js?v=1.2.3', array('ref'), null, true);
	} else if ( is_page('checkout') ) { // Checkout
		wp_enqueue_script('checkout', $directory . '/dist/checkout.bundle.js?v=1.2.3', array(), null, true);
	} else {
		wp_enqueue_script('general', $directory . '/dist/general.bundle.js?v=1.2.3', array(), null, true);
	}
}
